...
from thesis.energy import get_tariff, get_dispatch_controlled
from thesis.plotting import plot_day, plot_financial
from thesis.util import get_yaml


def pv_bess_controlled(input, config, best_solution, plot=False):
    # energy calculation
    ...
    for i in range(1, duration + 1):
        ...
        soc_profile_i, charge_profile_i, discharge_profile_i = get_dispatch_controlled(pv_gen_adjusted,
                                                                                       input["max_export"],
        ...
        if i == 1:
            ...
            if plot:
                plot_day(209, 211, input["max_export"], pv_gen_adjusted, soc_profile_y1, discharge_profile_y1,
                         charge_profile_y1)
...
if __name__ == '__main__':
    exp_folder = './output/scenario3_optim_20210826_122558'

    # get input and config data
    input = get_yaml(f"{exp_folder}/input.yaml")
    config = get_yaml(f"{exp_folder}/config.yaml")
    best_solution = np.loadtxt(f"{exp_folder}/best_solution.csv")
    pv_bess_controlled(input, config, best_solution, plot=True)
